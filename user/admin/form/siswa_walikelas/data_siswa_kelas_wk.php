<?php 
  ...
  $qcekwk = mysqli_query($conn, "SELECT b.nama_guru, a.status_wali_kelas, a.id_guru, b.nip, a.username, a.id_walikelas as id_walikelas
  from wali_kelas a left join guru b on a.id_guru = b.id_guru where a.username='$username' order by id_walikelas desc limit 1");
...
<hr>
  <div class="box-header with-border">
   ...
   <div class="pull-right">
     <a href="form/siswa_walikelas/print_nilai.php?id_kelas=<?php echo $id_kelas ?>&id_ta=<?php echo $id_ta ?>&semester=<?php echo $semester_ke ?>" class="btn btn-info btn-sm" target="_blank">Cetak</a>
   </div>
  </div>
<?php
...
?>
  ...
  <td><?php echo $data['jk']; ?></td>
   <?php 
    ...
    $id = $data['id_siswa'];
    $id_mapel = $dmapel['id_mapel'];
      $qnilai = mysqli_query($conn, "SELECT * from nilai where id_kelas='$id_kelas' and id_ta='$id_ta_aktif' 
                and id_wali_kelas='$id_wali_kelas' and id_mapel='$id_mapel' and id_siswa='$id' and semester='$semester_ke'") 
                or die(mysqli_error());
  ...
  <td>
    <?php if ($id_wk_aktif!=$id_wk) {
      if ($dcekwk['status_wali_kelas']=="0") {
       ...
       ?>
      <a href="?a=detail_siswa&id=<?php echo $data['id_siswa'] ?>
      &t=1&id_kelas=<?php echo $id_kelas ?>&id_ta=<?php echo $id_ta_aktif ?>
      &id_wk=<?php echo $id_wk ?>&id_wk_aktif=<?php echo $id_wali_kelas ?>
      &tingkat=<?php echo $data['tingkat'] ?>
      &status_ks=<?php echo $data['status_ks'] ?>&semester=<?php echo $semester_ke ?>&status_ta=<?php echo $status_ta ?>" class="btn btn-info btn-xs">
      <!-- <i class="fa fa-sort-numeric-asc"></i> --> Penilaian</a> 
